...
HPGe::HPGe *DetectorLibrary::GetDetector(const std::string &id, const std::string &position, const G4double filter) const
{
    ...
    if (_s == specifications.end()) {
        G4Exception("DetectorLibrary::GetDetector", "Unknown detector", FatalException, ("Detector " + id + " has no specification.").c_str());
    }
    ...
    case HPGe::tHEXAGONAL:
        ...
        break;
    default:
        G4Exception("DetectorLibrary::GetDetector", "Unknown detector type", FatalException, ("Detector " + id + " has an unknown type.").c_str());
        break;
    }
